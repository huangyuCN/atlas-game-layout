...
// gRPC 服务端（入队/取消/查询）+ HTTP 健康检查，供 bootstrap 收集。
package server
...
func NewGRPCServer(cfg *conf.Config) (transport.Server, error) {
	srv, err := atlasgrpc.NewServer(atlasgrpc.WithAddress(cfg.GRPC.Addr))
	if err != nil {
