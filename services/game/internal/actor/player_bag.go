// Package actor 背包/查询域业务方法：GrantItem/GetBackpack/GetPlayer（实现 PlayerServiceActorServer 接口
...
// GrantItem 实现 gamev1.PlayerServiceActorServer：聚合根 undo 写（未登录拒绝）。
...
// GetBackpack 实现 gamev1.PlayerServiceActorServer：背包查询（聚合根内存快照）。
...
// GetPlayer 实现 gamev1.PlayerServiceActorServer：玩家摘要查询（聚合根内存快照）。
...
// GetPlayerData 实现 gamev1.PlayerServiceActorServer：玩家数据全量快照（摘要 + 背包，
