// Package actor 认证域业务方法：Register/Login/Logout（实现 PlayerServiceActorServer 接口的认证部分）。
...
// Register 实现 gamev1.PlayerServiceActorServer（两段式注册）：
...
// Login 实现 gamev1.PlayerServiceActorServer：首登加载聚合根到内存（redis→mongo
...
// Logout 实现 gamev1.PlayerServiceActorServer（Tell，单向）：收到即保存并停止自身
